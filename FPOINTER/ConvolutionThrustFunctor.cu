...
__device__ fptype device_ConvolveSharedPdfs (fptype* evt, fptype* p, unsigned int* indices) { 
  ...
  for (int i = 0; i < numbins; i += numToLoad) {
    ...
    // a MappedThrustFunctor in which one of the other functions
...
__host__ void ConvolutionThrustFunctor::registerOthers (std::vector<ConvolutionThrustFunctor*> others) {
  ...
  for (unsigned int i = 0; i < others.size(); ++i) {
    ConvolutionThrustFunctor* curr = others[i];
