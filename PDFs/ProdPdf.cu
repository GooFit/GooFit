...
EXEC_TARGET fptype device_ProdPdfs (fptype* evt, fptype* p, unsigned int* indices) { 
  ...
  for (int i = 1; i < numParams; i += 2) {
    ...
    //if ((0 == THREADIDX) && (0 == blockIdx.x) && (gpuDebug & 1) && (paramIndices + debugParamIndex == indices))
    ...
    //printf("(%i, %i) device_Prod: (%f %f %f %f) %f %f %f %i\n", blockIdx.x, THREADIDX, evt[0], evt[8], evt[6], evt[7], curr, ret, normalisationFactors[parIdx], i);
    //printf("(%i, %i) device_Prod: (%f %f) %f %f %f %i\n", blockIdx.x, THREADIDX, evt[0], evt[1], curr, ret, normalisationFactors[parIdx], i);
