...
*/
// MEM_CONSTANT unsigned int AmpIndices_TD[100];
  ...
  , genhigh(5)
{
  ...
  for(int i=0; i<AmpsB.size(); i++){
    ...
    unsigned int flag = 1;
    auto inB = std::find_if(AmpBuffer.begin(), AmpBuffer.end(), [AmpsB,i](const Amplitude* A){return *(AmpsB[i]) == (*A);} );
...
__host__ void TDDP4::setDataSize (unsigned int dataSize, unsigned int evtSize) {
  ...
  MEMCPY_TO_SYMBOL(cResSF_TD, &dummy, sizeof(devcomplex<fptype>*), cacheToUse*sizeof(devcomplex<fptype>*), cudaMemcpyHostToDevice); 
  
  cachedAMPs = new DEVICE_VECTOR<devcomplex<fptype> >(dataSize*(AmpCalcs.size())); 
  void* dummy2 = thrust::raw_pointer_cast(cachedAMPs->data()); 
  MEMCPY_TO_SYMBOL(Amps_TD, &dummy2, sizeof(devcomplex<fptype>*), cacheToUse*sizeof(devcomplex<fptype>*), cudaMemcpyHostToDevice); 
...
EXEC_TARGET devcomplex<fptype> NormLSCalculator_TD::operator () (thrust::tuple<mcbooster::GReal_t, mcbooster::GReal_t, mcbooster::GReal_t, mcbooster::GReal_t, mcbooster::GReal_t> t) const {
  // Calculates the BW values for a specific resonance. 
  devcomplex<fptype> ret;
...
EXEC_TARGET thrust::tuple<fptype,fptype,fptype,fptype> NormIntegrator_TD::operator() (thrust::tuple<int, int, fptype*, devcomplex<fptype>*> t) const {
  unsigned int * indices = paramIndices + _parameters;
  unsigned int totalAMP = indices[5];
