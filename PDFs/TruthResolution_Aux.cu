...
EXEC_TARGET fptype device_truth_resolution_average_tau(fptype A2, fptype B2, fptype ABr, fptype ABi, fptype xmixing, fptype ymixing, fptype tau){
  fptype a = A2-B2;
  fptype b = 2*ABi;
  fptype c = A2+B2;
  fptype d = 2*ABr;
  fptype averagetau = ((xmixing*xmixing + 1)*(ymixing*ymixing - 1)*
              (((a*tau*(xmixing*xmixing - 1.) + 2.*b*tau*xmixing))/((xmixing*xmixing + 1.)*(xmixing*xmixing + 1.)) + (c*(-(tau*ymixing*ymixing) - tau) + 
             d *(2.*tau)*ymixing)/((ymixing*ymixing - 1.)*(ymixing*ymixing - 1.))))/((ymixing*ymixing - 1)*(b*xmixing - a) + (xmixing*xmixing + 1)*(c - d*ymixing));
