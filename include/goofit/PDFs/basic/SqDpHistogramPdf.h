...
class SqDpHistogramPdf : public GooPdf {
  public:
    SqDpHistogramPdf(std::string n, TH2* hist, Observable &mPrime, Observable &thPrime, Variable motherMass, Variable d1Mass, Variable d2Mass, Variable d3Mass, bool upperHalfOnly, bool eff);
    __host__ auto normalize() -> fptype override;
    __host__ void copyHistogramToDevice(thrust::host_vector<fptype> &host_histogram);
     __host__ void setVetos(std::vector<Variable> &Vetos);
