...
template <typename T> struct devcomplex {
    ...
    __device__ __host__ devcomplex<T> () : real(0), imag(0) {}
    __device__ __host__ devcomplex<T> (T r, T i) : real(r), imag(i) {}
...
template <typename T> __device__ devcomplex<T> operator/ (const devcomplex<T>& one, const devcomplex<T>& other) {
    ...
    return devcomplex<T>(inverse*(one.real*other.real+one.imag*other.imag),
                         inverse*(one.imag*other.real - one.real*other.imag));
}
...
template <typename T> __device__ devcomplex<T> operator* (const int& other, const devcomplex<T>& one) {
    return devcomplex<T>(one.real*other, one.imag*other);
}
...
template<typename T> __device__ devcomplex<T> operator* (const devcomplex<T>& one, const T& other) {
    return devcomplex<T>(one.real*other, one.imag*other);
}
