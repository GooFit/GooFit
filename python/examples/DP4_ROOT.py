...
#adapted exampled for generating time independent  4 body phasespace
...
Bose_symmetrized_AMP_S = Amplitude( "K*(892)rho(770)_S", Variable("amp_real1", -0.115177),Variable("amp_imag1", 0.153976), LSKRS, SFKRS, 2)
Bose_symmetrized_AMP_P = Amplitude( "K*(892)rho(770)_P", Variable("amp_real2", -0.0298697),Variable("amp_imag2", -0.0722874), LSKRP, SFKRP, 2)
Bose_symmetrized_AMP_D = Amplitude( "K*(892)rho(770)_D", Variable("amp_real3", -0.452212),Variable("amp_imag3", 0.426521), LSKRD, SFKRD, 2)        
...
#dataframe to store the variables and write to root file                                                                                                                                                                                                                              
m12arr = []
m34arr = []
c12arr = []
c34arr = []
...
data_dict = {'m12':m12arr,'m34':m34arr,'c12':c12arr,'c34':c34arr,'phi':phiarr}
...
for i in range(len(variables[0])):
    if(flags[i] == True):
        ...
        c34arr.append(variables[3][i])
        phiarr.append(variables[4][i])
        #dtimearr.append(variables[5][i])
                
df = pd.DataFrame(data_dict)

print("Generate {} events".format(len(df.index)))
df.to_root("output_TID.root")
