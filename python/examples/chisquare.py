...
def main():
    ...
    dZeroLinearCoef = (
        magPQ
        * np.sqrt(rSubD)
        * (y_mix * np.cos(delta + wpPhi) - x_mix * np.sin(delta + wpPhi))
    )
    d0barLinearCoef = (
        magQP
        * np.sqrt(rBarD)
    ...
    retval1 = fitRatio(
        ...
        dZeroEvtsRS,
        dZeroEvtsWS,
        "chisquare_dzeroEvtRatio_goo_python.png",
    )
    retval2 = fitRatio(
        decayTime,
        weights,
        d0barEvtsRS,
        d0barEvtsWS,
        "chisquare_dzbarEvtRatio_goo_python.png",
    )
    ...
    with timed("Total CPU (2x fits)"):
        fitRatioCPU(
            ...
            dZeroEvtsRS,
            dZeroEvtsWS,
            "chisquare_dzeroEvtRatio_cpu_python.png",
        )
        fitRatioCPU(
            decayTime,
            d0barEvtsRS,
            d0barEvtsWS,
            "chisquare_dzbarEvtRatio_cpu_python.png",
        )
