...
def cpuGetM23(massPZ, massPM):
    ...
    return (
        _mD02
        + piZeroMass * piZeroMass
        + piPlusMass * piPlusMass
        + piPlusMass * piPlusMass
        - massPZ
        - massPM
    )
...
def getToyData(toyFileName):
    ...
    out = pd.read_csv(
        ...
        names=["event", "m23", "m12", "m13", "e23", "e12", "e13", "tau", "sigma_t", "mD0", "deltaM", "probSig", "dst_charge", "run", "event2", "sig", "frac1", "frac2", "frac3", "frac4"],
    )
    out["vals"] = out.index
...
def makeSignalPdf(m12, m13, eventNumber, eff=None, fitMasses=False):
    ...
    nonr = Resonances.NonRes(
        "nonr",
        (
            Variable("nonr_amp_real", 0.5595 * (-1))
            if fixAmps
            else Variable("nonr_amp_real", 0.5595 * (-1), 0.001, 0, 0)
        ),
        (
            Variable("nonr_amp_imag", -0.108761 * (-1))
            if fixAmps
            else Variable("nonr_amp_imag", -0.108761 * (-1), 0.1, 0, 0)
        ),
    )
