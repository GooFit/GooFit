...
def getData(filename, tree, catIndex):
    df = read_root(filename, tree, columns=['m12', 'm13', 'sig', '__index__', 'B_DTFKS_D0_CTAU', 'B_DTFKS_D0_M'])
 ...
 #modes = ['SingleTag_D0ToKsPiPiLL', 'SingleTag_D0ToKsPiPiDD']
  ...
  for year in years:
    for mode in modes:
      ...
      signal_pdf= Amp3Body(f'signalPDF_{yearmode}', m12, m13, eventNumber, globaldecinfo, efficiency)
      ...
      total_pdfs[yearmode] = EventWeightedAddPdf(f'total_{yearmode}', [sigprob], [signal_pdf, BGPdf])
  print(total_pdfs)
  ...
  for param in finalPDF_comb.getParameters():
    if 'mass' in param.getName() or 'width' in param.getName():
        if fixMass: param.setFixed(True)
    if 'beta' in param.getName() or 'f_prod' in param.getName():
        if fixKmatrix: param.setFixed(True) # Fix K-matrix parameters
  ...
  for column in params_df.columns:
      for param in finalPDF_comb.getParameters():
          if column == param.getName(): 
            print(f'setting parameter {param.getName()} from {param.getValue()} to {params_df[column][0]}')
  ...
  if random_seed > -1:
    ...
    for param in finalPDF_comb.getParameters():
      if not param.IsFixed():
        ...
        print(f'resampling parameter {param.getName()} from {param.getValue()} to {resampled_param}')
  ...
  for year in years:
    for mode in modes:
      print("offset", offset)
      signal_pdfs[f'{year}_{mode}'].setDataSize(datasets[f'{year}_{mode}'].getNumEvents(), 5, offset)
  ...
  for year in years:
    for mode in modes:
      ...
      print([signal_pdfs[f'{year}_{mode}'].fit_fractions()[i][i] for i in range(len(globaldecinfo.resonances))])
      print(f'Sum of fit fractions {year} {mode}: ', np.diag(signal_pdfs[f'{year}_{mode}'].fit_fractions()).sum())
      makePlots(total_pdfs[f'{year}_{mode}'], signal_pdfs[f'{year}_{mode}'], globaldecinfo, datasets[f'{year}_{mode}'], mode, year, plotprefix)
...
def main():
  ...
  all_modes = ['SingleTag_D0ToKsPiPiLL', 'SingleTag_D0ToKsPiPiDD', 'DoubleTag_D0ToKsPiPiLL', 'DoubleTag_D0ToKsPiPiDD']
