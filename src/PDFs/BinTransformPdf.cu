...
__device__ fptype device_BinTransform(fptype* evt, fptype* p, unsigned int* indices) {
    ...
    for(int i = 0; i < numObservables; ++i) {
        ...
        int localBin = static_cast<int>( floor((obsValue - lowerLimit) / binSize));
...
__host__ BinTransformPdf::BinTransformPdf(std::string n, std::vector<Variable*> obses,
    ...
    MEMCPY_TO_SYMBOL(functorConstants, host_constants, 2*obses.size()*sizeof(fptype), cIndex*sizeof(fptype),
