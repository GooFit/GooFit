...
__device__ fptype device_InterHistogram(fptype* evt, fptype* p, unsigned int* indices) {
    ...
    for(int i = 0; i < totalBins; ++i) {
        ...
        currentWeight = currentWeight > 0 ? (currentWeight <= sqrt(static_cast<fptype>( numVars)) ? 1 / sqrt(currentWeight) : 0) : 0;
...
__host__ InterHistPdf::InterHistPdf(std::string n,
    ...
    MEMCPY_TO_SYMBOL(functorConstants, host_constants, numConstants*sizeof(fptype), cIndex*sizeof(fptype),
    ...
    MEMCPY_TO_SYMBOL(dev_base_interhists, dev_address, sizeof(fptype*), totalHistograms*sizeof(fptype*),
                     cudaMemcpyHostToDevice);
