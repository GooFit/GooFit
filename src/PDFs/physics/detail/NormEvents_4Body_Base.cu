...
namespace GooFit {
  ...
  __host__ bool NormEvents_4Body_Base::computeCachedLSValuesForBatch_TD(
									...
									mcbooster::mc_device_vector<fpcomplex>& ls_batchResult_d)							   
 {
   ...
   for(int i = 0; i < lsFunctionIndices.size(); ++i)
   {
     ...
     thrust::transform(
		       thrust::make_zip_iterator(thrust::make_tuple(batchNormM12_d.cbegin(),
 ...
 __host__ bool NormEvents_4Body_Base::computeCachedSFValuesForBatch_TD(
								       ...
								       mcbooster::RealVector_d& sf_batchResult_d)
 {
  ...
  __host__ fptype NormEvents_4Body_Base::doNormIntegral_TD(
							 ...
							 unsigned int dalitzId,
							 unsigned int numAccThisBatch,
							 mcbooster::RealVector_d& batchSF_d, //not modified, can't seem to make const type work with thrust::transform_reduce
							 mcbooster::mc_device_vector<fpcomplex>& batchLS_d) //not modified, can't seem to make const type work with thrust::transform_reduce
  {
