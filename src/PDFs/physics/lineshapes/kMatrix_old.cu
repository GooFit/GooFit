#include <goofit/PDFs/physics/lineshapes/kMatrix_old.h>

#include <goofit/PDFs/ParameterContainer.h>
#include <goofit/PDFs/physics/kMatrixUtils_old.h>
...
__device__ fpcomplex kMatrixFunction(fptype Mpair, fptype m1, fptype m2, ParameterContainer &pc) {
    ...
    phaseSpace << phsp_twoBody_old(s, mPiPlus, mPiPlus), phsp_twoBody_old(s, mKPlus, mKPlus), phsp_fourPi_old(s),
        phsp_twoBody_old(s, mEta, mEta), phsp_twoBody_old(s, mEta, mEtap);

    Eigen::Array<fpcomplex, NCHANNELS, NCHANNELS> F = getPropagator_old(kMatrix, phaseSpace, adlerTerm);
