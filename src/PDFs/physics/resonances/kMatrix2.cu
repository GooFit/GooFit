#include <goofit/PDFs/physics/resonances/kMatrix2.h>
...
template <int I>
__device__ auto plainkMatrix2(fptype m12, fptype m13, fptype m23, ParameterContainer &pc) -> fpcomplex {
...
__device__ resonance_function_ptr ptr_to_kMatrix2     = plainkMatrix2<1>;
__device__ resonance_function_ptr ptr_to_kMatrix2_Sym = plainkMatrix2<2>;

namespace Resonances {

kMatrix2::kMatrix2(std::string name,
         Variable ar,
         Variable ai,
         Variable mass,
         Variable width,
         ...
         bool norm,
         bool sym)
    : ResonancePdf("kMatrix2", name, ar, ai) {
    ...
    if(sym)
        registerFunction("ptr_to_kMatrix2_Sym", ptr_to_kMatrix2_Sym);
    else
        registerFunction("ptr_to_kMatrix2", ptr_to_kMatrix2);
}
