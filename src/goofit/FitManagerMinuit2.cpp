...
std::vector <std::vector<fptype>> FitManagerMinuit2::printParams()
{
    ...
    std::cout << "*Note: this fuction runs over all free parameters, if you have other parameters than real and imaginary coefs fix them before call this function!  \n";
	...
	for(int i = 0; i < floatVarVal.size(); i+=2){
		...
		fptype mag_err = dmda(a,b)*dmda(a,b)*matCov(i,i)
		...
		fptype phi_err = dpda(a,b)*dpda(a,b)*matCov(i,i)
		...
		std::cout << std::fixed << "Resonance " << (i+2)/2 << "\t" << mag << "\t" << mag_err << "\t" << phi << "\t" << phi_err << std::endl;
	}

    std::cout << "" << std::endl;

	std::vector <std::vector<fptype>> ret; ret.clear();
