...
//*-* debug printout:
   ...
   if (fabs(fnext - aim) < tlf) goto L800;
...
L460:
   ...
   tla = .01;
   if (fabs(aopt) > 1) tla = fabs(aopt)*.01;
...
L160:
      ...
      for (i1 = l; i1 <= m1; ++i1) {
         ...
         gl = c*work[n + i];
         h  = c*pt;
         if (fabs(pt) >= fabs(work[n + i])) goto L180;
...
L901:
   ...
   for (ilist = 1; ilist <= llist; ++ilist) {
      ...
      continue;
L930:
      if (fISW[4] >= 0) printf(" PARAMETER %4d %s IGNORED.\n",iext,chwhy.c_str());
   }
...
//*-*-                   get MINUIT-calculated first derivatives
   ...
   for (lc = 1; lc <= fNpar; ++lc) {
      ...
      cwd = "GOOD";
      err = fDgrd[lc-1];
      if (fabs(fGRADgf[lc-1] - fGrd[lc-1]) > err)  cwd = " BAD";
      if (fGin[i-1] == fUndefi) {
         cwd      = "NONE";
         lnone    = true;
         fGRADgf[lc-1] = 0;
...
L30:
         ...
         if (fGstep[i-1] > 0) fGstep[i-1] =  fabs(d);
         else                 fGstep[i-1] = -fabs(d);
         fDirin[i-1] = d;
         fHESSyy[i-1]= fs1;
         dlast       = d;
...
//*-*-        if parameter has limits, max int step size = 0.5
         stpinm = .5;
         if (fGstep[i-1] < 0) d = min(d,stpinm);
         if (d < dmin_) d = dmin_;
//*-*-          see if converged
...
//*-*-                 decrease step until first derivative changes by <5%
         ...
         if (fabs(grdold - grdnew) < dgmin) goto L60;
         if (d < dmin_) {
...
L100:
   ...
   for (i = 1; i <= fNpar; ++i) {
      ...
      if (fabs(fX[i-1] - fXt[i-1]) > fDirin[i-1])     goto L150;
   }
   goto L230;
L150:
   ...
   for (i = 1; i <= fNpar; ++i) {
      ...
      if (fabs(fX[i-1] - fXt[i-1]) > fDirin[i-1])     goto L250;
   }
L230:
...
//*-*-                          do line search if second derivative negative
   ...
   if (lined2 < (fIstrat + 1)*fNpar) {
      for (i = 1; i <= fNpar; ++i) {
         ...
         if (fGrd[i-1] > 0) fMIGRstep[i-1] = -fabs(fGstep[i-1]);
         else               fMIGRstep[i-1] =  fabs(fGstep[i-1]);
...
//*-*-        stop if already at limit with negligible step size
      if (fabs(delu) / (fabs(ut) + fabs(fU[ilax-1])) < fEpsmac) goto L440;
...
//*-*-             report function value and status
   ...
   else                 cheval = local_Form("%g",fval).c_str();

   if (fEDM == fBigedm) chedm = " unknown  ";
   else                 chedm = local_Form("%g",fEDM).c_str();
...
//*-*-             variable parameter.
      ...
      if (cx2 == "PLEASE GET X..")  cx2 = local_Form("%14.5e",x2).c_str();
      if (cx3 == "PLEASE GET X..")  cx3 = local_Form("%14.5e",x3).c_str();
...
//*-*-          Command could be SET xxx, SHOW xxx,  HELP SET or HELP SHOW
   ...
   if ( contains(ctemp2, "HEL"))  goto L2000;
   if ( contains(ctemp2, "SHO"))  goto L1000;
   if (!contains(ctemp2, "SET"))  goto L1900;
//*-*-                          ---
   ckind = "SET ";
//*-*-                                       . . . . . . . . . . set unknown
   ...
   if (fISW[1] >= 1) {
      ...
      if (ierr == 0) {
         for (iin = 1; iin <= fNpar; ++iin) {
            ...
            else                          fGlobcc[iin-1] = sqrt(1 - 1 / denom);
         }
      }
   }
} /* mnwerr_ */
