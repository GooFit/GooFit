...
TEST_CASE("Variations on a exp fit", "[simple][fit][exp]") {
    ...
    double err = 1 / sqrt(N) * fabs(alpha.getValue()) CHECK(alpha.getError() == Approx(err).epsilon(0.1));
...
TEST_CASE("Variations on a dual fit", "[simple][fit][exp]") {
    ...
    CHECK(xalpha.getError() == Approx(xerr).epsilon(0.1));
    CHECK(yalpha.getError() == Approx(yerr).epsilon(0.1));
